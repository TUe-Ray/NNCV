...
# 1️⃣ 先把想用的權重寫成常數，方便日後切換
MODEL_NAME = "nvidia/mit-b5"          # ← or "nvidia/segformer-b5-finetuned-ade-640-640"
...
def main(args):
   ...
   # 載入預訓練的 config，然後修改 num_labels
    # 3️⃣ config 與 model 都指向同一個 MODEL_NAME
    config = SegformerConfig.from_pretrained(
        MODEL_NAME,
        num_channels=3,
        num_labels=19,           # Cityscapes 類別數
        upsample_ratio=1
    )
    model = SegformerForSemanticSegmentation.from_pretrained(
        MODEL_NAME,
        config=config,
        ignore_mismatched_sizes=True   # 讓 decode head 可自動重建
    ).to(device)
